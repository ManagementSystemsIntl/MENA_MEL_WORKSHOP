...
-   Understand the key validity threats to Randomized Controlled Trials (RCTs), and where those threats may be addressed in the management cycle
...
$=E\bigr[Y^1\bigr]-\bigr[Y^0\bigr]$ --> Average Treatment Effect

$+E\bigr[Y^0|D=1\bigr]-E\bigr[Y^0|D=0\bigr]$ --> Selection bias

$+(1-\pi)(ATT-ATU)$ --> Heterogeneous treatment effects^[Let's not talk about this just yet]
...
If we knew the prior level of knowledge, we can be more sure about our treatment effect estimates^[AND we would identify heterogeneous treatment effects if they existed]
